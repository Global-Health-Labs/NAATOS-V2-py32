...
void pid_controller_compute(heater_t heater, float measurement) {
  ...
  // Compute Integral
  pid_data[heater].integrator += error;
  ...
  float d_input = measurement - pid_data[heater].prevMesurement;
  // Compute PID Output
  float out = pid_data[heater].k_p * error + pid_data[heater].k_i * pid_data[heater].integrator - pid_data[heater].k_d * d_input;
