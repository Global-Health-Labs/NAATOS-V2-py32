...
float sTune::softPwm(GPIO_TypeDef gpiox, const uint16_t relayPin, float input, float output, float setpoint, uint32_t windowSize, uint8_t debounce) {
  ...
  if (!relayStatus && optimumOutput > (msNow - windowStartTime)) {
    if (msNow > nextSwitchTime) {
      ...
      relayStatus = true;
      HAL_GPIO_WritePin(gpiox, relayPin, 01U);
    ...
    if (msNow > nextSwitchTime) {
      nextSwitchTime = msNow + debounce;
      relayStatus = false;
      HAL_GPIO_WritePin(gpiox, relayPin, 00U);
